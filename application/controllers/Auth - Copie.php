	...
	public function index()
	{
     //$this->session->set_flashdata('users', $this->ion_auth->users()->result());
		...
		else if (!$this->ion_auth->is_admin()) // remove this elseif if you want to enable this for non-admins
		{
			...
			return show_error('vous devez etre admin pour voir cette');
		}
		else
		{
	...
	public function login()
	{
		...
		if ($this->form_validation->run() === TRUE)
		{
			...
			else
			{
				...
				$this->layout->view('auth/login', 'refresh'); // use redirects instead of loading views for compatibility with MY_Controller libraries
			}
		}
		else
		{
